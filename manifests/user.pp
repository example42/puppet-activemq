...
class activemq::user inherits activemq {
  $user_before = $activemq::install ? {
    source  => "[Group[$activemq::process_group], Puppi::Netinstall['netinstall_activemq']]",
    puppi   => "[Group[$activemq::process_group], Puppi::Project::Tar['activemq']]",
    default => undef,
  }
  ...
  Group <| title == $activemq::process_group |>
